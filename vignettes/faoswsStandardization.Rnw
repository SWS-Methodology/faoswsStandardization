...
## DELETE!  JUST TESTING:
files = dir("~/Documents/Github/faoswsStandardization/R", full.names = TRUE)
sapply(files, source)
...
<<>>=
adjustedTree = faoswsStandardization:::adjustCommodityTree(commodityTree = copy(tree), nSigma = 1.5,
...
data = addMissingElements(usaWheat, params)
...
print(printSUATable(data = data, standParams = params,
...
<<>>=
toPrint = markUpdated(new = data, old = old, standParams = params)
printSUATable(data = toPrint, standParams = params,
              printCodes = printCodes)
old = copy(data)
@
...
tree = collapseEdges(edges = tree, parentName = params$parentVar,
                     childName = params$childVar,
                     extractionName = params$extractVar,
                     keyCols = NULL)
...
tree = tree[, list(share = sum(share),
            ...
            by = c("measuredItemChildCPC", "measuredItemParentCPC",
                   "geographicAreaM49", "timePointYearsSP",
                   "extractionRate", "target")]
...
save(fbsTree, file = "~/Documents/Github/faoswsStandardization/data/fbsTree.RData")
@
