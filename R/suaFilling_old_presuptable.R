...
suaFilling_old_presuptable = function(data, p = p, tree=tree,
                      primaryCommodities = c(), stockCommodities = c(),
                      debugFile= NULL,
                      utilizationTable=c(),supplyTable = c(), 
...
#       data=rbind(dataNoImbP, dataNegImbP, dataPosImbP,dataNoImb)
#     }else{
#       if("newValue" %in% colnames(dataPosImb)){
#         dataPosImb[!is.na(newValue),Value:=newValue]
#         data=rbind(dataNoImbP, dataNegImbP, dataPosImbP,dataNoImb,dataPosImb[,1:17,with=FALSE])
#       }else{
#         data=rbind(dataNoImbP, dataNegImbP, dataPosImbP,dataNoImb,dataPosImb[,1:17,with=FALSE])
#       }
#     }
...
#     }
#   if(dim(dataPosImb)[1]==0){
#     data=rbind(dataNoImbP, dataNegImbP, dataPosImbP,dataNoImb,dataNegImb[,1:17,with=FALSE])
#   }else{
#     if("newValue" %in% colnames(dataPosImb)){
#       dataPosImb[!is.na(newValue),Value:=newValue]
#       data=rbind(dataNoImbP,dataNegImbP, dataPosImbP[,1:17,with=FALSE],dataNoImb,dataNegImb[,1:17,with=FALSE],dataPosImb[,1:17,with=FALSE])
#     }else{
#   data=rbind(dataNoImbP, dataNegImbP, dataPosImbP,dataNoImb,dataNegImb[,1:17,with=FALSE],dataPosImb[,1:17,with=FALSE])
#     } 
#   }
# }
