...
##' @param supplyTable Table of utilization for suaFilling
...
standardizationWrapperSupTable = function(data, tree, fbsTree = NULL, standParams,
    ...
    # utilizationTable[,SU:=U]
    # supplyTable[,SU:=S]
    
    rankTable = rbind(utilizationTable,supplyTable)
    #  here ranks are separated for supply and utilizations even with no additional variable
    
    data=data.table(left_join(data,rankTable,by=c("geographicAreaM49","measuredElementSuaFbs","measuredItemSuaFbs")))
    
    data = unique(data)
    
    dataNoF=suaFilling(data, p=p, tree=tree,
                    primaryCommodities = primaryEl, debugFile=p$createIntermetiateFile,
                    stockCommodities = stockCommodities,
                    rankTable = rankTable, imbalanceThreshold = 10,loop1=TRUE,NoFillable=data.table())

    data = dataNoF$data
    NoFillable = dataNoF$NoFillable
    ...
    ### STEP 3: Compute availability and SHARE 1 
      ...
      #### SHAREs 1 
         ...
         dataNoF=suaFilling(data, p=p, tree=tree,
                         ...
                         rankTable = rankTable, imbalanceThreshold = 10,loop1=FALSE,NoFillable=NoFillable)
