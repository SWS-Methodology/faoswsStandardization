...
checkShareValue=function(tree2Subset){
  ...
  fun5=function(tree2Subset,case="2.A.a.1"){
    ...
    tree2Subset[,message:="all protected, sum!=0, all recalculated"]
    return(tree2Subset)
    }
  ...
  fun6=function(tree2Subset,case="2.A.a.2"){
    ...
    tree2Subset[(availability.child<=0|is.na(availability.child)),message:="all protected, neg Availability, sum!=0, all recalculated"]
    
    tree2Subset[availability.child>0,severity:=2]
    tree2Subset[availability.child>0,message:="all protected, sum!=0, all recalculated"]
    return(tree2Subset)
    }
  ...
  fun8=function(tree2Subset,case="2.A.b.2"){ 
    ...
    tree2Subset[is.na(share)&availability.child>0,message:="all protected, missing share, sum!=0, recalculated"]
    tree2Subset[is.na(share)&(availability.child<=0|is.na(availability.child)),message:="all protected, missing share, neg Avail, sum!=0, recalculated"]
    
    tree2Subset[!(is.na(share))&availability.child>0,severity:=2]
    tree2Subset[!(is.na(share))&(availability.child<=0|is.na(availability.child)),severity:=2]
    
    
    tree2Subset[!(is.na(share))&availability.child>0,message:="all protected, sum!=0, recalculated"]
    tree2Subset[!(is.na(share))&(availability.child<=0|is.na(availability.child)),message:="all protected, sum!=0, neg Avail, recalculated"]
    return(tree2Subset)
    }
  ...
  fun9=function(tree2Subset,case="2.B.a.1.i"){
    ...
    if(shareProt>1){
      ...
      tree2Subset[checkFlags!="(E,f)",message:="also protected shares recalculated"]
    }else{
      ...
      tree2Subset[checkFlags!="(E,f)",message:="only not protected shares have been recalculated"]
      
      tree2Subset[checkFlags=="(E,f)",severity:=as.integer(0)]
      tree2Subset[checkFlags=="(E,f)",message:="protected share"]
    }
    return(tree2Subset)
  }
  ...
  fun10=function(tree2Subset,case="2.B.a.1.ii"){ 
    ...
    if(shareProt>1){
      ...
      tree2Subset[checkFlags=="(E,f)"&!(is.na(share)),message:="valid protected share"]
      
      tree2Subset[checkFlags=="(E,f)"&is.na(share),severity:=3]
      tree2Subset[checkFlags=="(E,f)"&is.na(share),message:="Missing protected share recalculated"]
  ...
  fun11=function(tree2Subset,case="2.B.a.2.i"){ 
    ...
    if(shareProt>1){
      ...
      tree2Subset[checkFlags=="(E,f)"&(availability.child<=0|is.na(availability.child)),message:="sum of prot shares>1 & neg Availability, recalculated"]
      ...
      tree2Subset[checkFlags=="(E,f)"&(availability.child<=0|is.na(availability.child)),message:="protected share with negative availability"]
      
      tree2Subset[checkFlags=="(E,f)"&availability.child>0,severity:=as.integer(0)]
      tree2Subset[checkFlags=="(E,f)"&availability.child>0,message:="protected share"]
    }
    return(tree2Subset)
  }
  ...
  fun12=function(tree2Subset,case="2.B.a.2.ii"){ 
    ...
    if(shareProt>1){
      ...
      tree2Subset[!(checkFlags=="(E,f)")|(is.na(share))&
                    (availability.child<=0|is.na(availability.child)), negShare:=1/freq]
      
      tree2Subset[!(checkFlags=="(E,f)")|(is.na(share))&
                    (availability.child<=0|is.na(availability.child)),
      ...
      tree2Subset[!(checkFlags=="(E,f)")|(is.na(share))&
                    (availability.child<=0|is.na(availability.child)), 
                  newShare := ifelse(tempAvailability==0|is.na(tempAvailability),negShare,
      ...
      tree2Subset[checkFlags=="(E,f)"&!(is.na(share))&(availability.child<=0|is.na(availability.child)),message:="valid protected share, neg availability"]
      
      tree2Subset[checkFlags=="(E,f)"&!(is.na(share))&availability.child>0,severity:=0]
      tree2Subset[checkFlags=="(E,f)"&!(is.na(share))&availability.child>0,message:="valid protected share"]
      
      tree2Subset[checkFlags=="(E,f)"&is.na(share)&(availability.child<=0|is.na(availability.child)),severity:=3]
      tree2Subset[checkFlags=="(E,f)"&is.na(share)&(availability.child<=0|is.na(availability.child)),message:="Missing protected share with neg avail, recalculated"]
      
      tree2Subset[checkFlags=="(E,f)"&is.na(share)&availability.child>0,severity:=3]
      tree2Subset[checkFlags=="(E,f)"&is.na(share)&availability.child>0,message:="Missing protected share recalculated"]
  ...
  fun16=function(tree2Subset,case="2.B.b.2.ii"){ 
    ...
    if(shareProt>1){
      ...
      tree2Subset[checkFlags=="(E,f)"&(availability.child<=0|is.na(availability.child))&!(is.na(share)),message:="Negative, availability, valid protected share"]
      
      tree2Subset[checkFlags=="(E,f)"&(availability.child<=0|is.na(availability.child))&is.na(share),severity:=2]
      tree2Subset[checkFlags=="(E,f)"&(availability.child<=0|is.na(availability.child))&is.na(share),message:="Neg Availability, Missing protected share, recalculated"]
      
      tree2Subset[checkFlags=="(E,f)"&(availability.child>0)&!(is.na(share)),severity:=1]
      tree2Subset[checkFlags=="(E,f)"&(availability.child>0)&!(is.na(share)),message:="valid protected share"]
      
      tree2Subset[checkFlags=="(E,f)"&(availability.child>0)&is.na(share),severity:=3]
      tree2Subset[checkFlags=="(E,f)"&(availability.child>0)&is.na(share),message:="Missing protected share, recalculated"]
