...
RemainingToProcessedParent<-function(data){
  ...
  data<-data%>% group_by(geographicAreaM49,measuredItemParentCPC,timePointYears) %>%
    mutate(parent_already_processed=ifelse(is.na(parent_qty_processed),
                                           parent_qty_processed,
                                           sum(processed_to_child/extractionRate,na.rm = TRUE))) %>%
    mutate(remaining_processed_parent=round(parent_qty_processed-parent_already_processed))%>%
    mutate(remaining_processed_parent=ifelse(remaining_processed_parent<0,0,remaining_processed_parent)) %>%
    #in case we have only one child left(new) we assign the remaing parent quantity to process
    mutate(only_child_left=ifelse(sum(is.na(processed_to_child))==1 & 
                                    is.na(processed_to_child) &
                                    !is.na(production_of_child) &
                                    !is.na(parent_qty_processed) & 
                                    production_of_child>0,TRUE,FALSE)) %>%
    mutate(processed_to_child=ifelse(only_child_left==TRUE,remaining_processed_parent*extractionRate,processed_to_child)) %>%
...
RemainingProdChildToAssign<-function(data){
  ...
  data<-data %>% group_by(geographicAreaM49,measuredItemChildCPC,timePointYears) %>%
    mutate(available_processed_child=sum(processed_to_child,na.rm = TRUE)) %>%
    mutate(remaining_to_process_child=round(production_of_child-available_processed_child)) %>%
    mutate(remaining_to_process_child=ifelse(remaining_to_process_child<0,0,remaining_to_process_child)) %>%
    #in case we have one parent left
    mutate(only_parent_left=ifelse(sum(is.na(processed_to_child))==1 & 
                                     is.na(processed_to_child) &
                                     !is.na(parent_qty_processed) & 
                                     parent_qty_processed>=0,TRUE,FALSE)) %>%
    mutate(processed_to_child=ifelse(only_parent_left==TRUE,remaining_to_process_child,processed_to_child)) %>%
...
data_tree <- data[measuredElementSuaFbs %chin% c('production', 'imports', 'exports', 
                                                 'stock_change','foodmanufacturing')]
...
data_tree<-data_tree %>% select(measuredItemParentCPC,geographicAreaM49,timePointYears,
...
data_tree<-data_tree %>% left_join(data_count) %>%
  ...
  distinct(geographicAreaM49,measuredItemParentCPC,measuredItemChildCPC,
           timePointYears,extractionRate,parent_qty_processed,processingLevel,number_of_parent,number_of_children)



#extract production data for chil commodities
...
dataprodchild<-as.data.frame(dataprodchild)

dataprodchild<-dataprodchild %>% distinct(geographicAreaM49,measuredItemChildCPC,
                                          timePointYears,Value,flagObservationStatus,
                                          flagMethod) %>%
  rename(production_of_child=Value)

data_tree<-data_tree%>% left_join(dataprodchild) %>% 
  ...
  #mutate(processed_to_child=ifelse(number_of_parent==1,production_of_child/extractionRate,NA_real_)) %>%
  
  mutate(processed_to_child=ifelse(number_of_parent==1,production_of_child,NA_real_)) %>%
  ...
  mutate(processed_to_child=ifelse(number_of_children==1,parent_qty_processed*extractionRate,processed_to_child)) %>%
  mutate(processed_to_child=ifelse(production_of_child==0,0,processed_to_child))
for(k in 1:3){
...
data_tree<-data_tree %>% group_by(geographicAreaM49,measuredItemChildCPC,timePointYears) %>%
  
  mutate(processed_to_child=ifelse(number_of_parent>1 & is.na(processed_to_child),(remaining_to_process_child*is.na(processed_to_child)*remaining_processed_parent)/
                                     sum((remaining_processed_parent*is.na(processed_to_child)),na.rm = TRUE),
                                   processed_to_child)) %>%
  ungroup() %>%
  group_by(geographicAreaM49,measuredItemParentCPC,timePointYears) %>%
  mutate(parent_already_processed=sum(processed_to_child,na.rm = TRUE)) %>%
  mutate(remaining_processed_parent=round(parent_qty_processed-parent_already_processed)) %>%
  mutate(remaining_processed_parent=ifelse(remaining_processed_parent<0,0,remaining_processed_parent)) %>%
  ungroup()%>%
  group_by(geographicAreaM49,measuredItemChildCPC,timePointYears) %>%
  mutate(shareDownUp=processed_to_child/sum(processed_to_child,na.rm = T))%>%

  #it it necessary to correct some cases
  mutate(shareDownUp=ifelse(is.na(shareDownUp) & number_of_parent==1,1,shareDownUp))%>%
  mutate(shareDownUp=ifelse((production_of_child==0 | is.na(production_of_child)) & timePointYears<2014
                            ,0,shareDownUp))%>%
  mutate(shareDownUp=ifelse((parent_qty_processed==0 | is.na(parent_qty_processed)) & timePointYears<2014,
                            0,shareDownUp))%>%
  ...
  mutate(ShareDownUp_avg=rollavg(shareDownUp, order = 3))%>%
  ungroup() %>%
  group_by(geographicAreaM49,measuredItemChildCPC,timePointYears) %>%
  mutate(ShareDownUp_avg=ShareDownUp_avg/sum(ShareDownUp_avg,na.rm = TRUE)) %>%
  mutate(shareDownUp=ifelse(timePointYears>2013,ShareDownUp_avg,shareDownUp))

data_tree<-ExtrRate%>% select(geographicAreaM49,measuredItemParentCPC,measuredItemChildCPC,timePointYears) %>%
  left_join(data_tree)%>%mutate(shareDownUp=ifelse(is.na(shareDownUp),0,shareDownUp)) %>%
