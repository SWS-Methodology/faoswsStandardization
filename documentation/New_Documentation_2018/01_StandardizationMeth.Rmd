...
t9_cap <- table_nums(name = "t9", 
                        caption = "Utilization Table Bran ofWheat China Mainland 2014")
...
From `r table_nums("t4",display = "cite")` and `r table_nums("t5",display = "cite")` here is, as example, the computation of *Food procesing* of Wheat,Flour of wheat and Starch of wheat: 
...
1. If all the ranked utilizations are already filled, all of them are increased proportionally to their values. *Bran of Wheat* in our example, belongs to this case. Utilization table for Bran of Wheat is reported in `r table_nums("t8",display = "cite")`
...
According to this table, there aer 3 Utilizations that are suppose to be active for this commodity: *feed*, *food* and *exports*. *Exports* is excluded from this step, therefore, food and feed reamain. Both Variables are filled in the SUA line of This Commodity, therefore, as 
