...
abstract: |
  This vignette provides a description of the Workflow of the Standardization and Balancing Procedure: This represents the process of aggregating and balancing all the accounts of individual products to their primary equivalents. Other documents will cover technical aspects.
...
where the $i$ index runs over all countries, the $j$ index over all commodities, and $t$ over years and wgere, dropping indices for brevity:
...
The stadardized table is called *Fbs Standardized* and is composed of more than 1 line, because, besides the primary commodity thare are 2 more typologies of commodities, both : 

 * *No-tree* commodities, that are also *"zero-level"* commodities, appear here as they were in the previous step. Indeed, as these are not procesed, the standardization does not change any of the element's values.
...
The balancing of Standardized Sua in the new framework is based on the idea that each variable of equation \ref{eq:balance1} includes a measurement error, so that the equation can be rewritten as follows: 
...
and where each *adjustment ($adj_{el \in E}$)* is an balancing adjustment proportional to the measurement error (*ME*) of each element $el \in E = \{P,I,X,\Delta St, FP,Fo,Fe,Lo,Se,IU,T,ROU\}$

The balancing mechanism is a two step process aimed at modify each value proportionally to the *ME*. It start with the definition of the percentages of *Measurement Errors ($p_{ME}$)* for each element as reported in the following Table:

```{r t21, fig.cap=t21_cap, fig.pos = "H"}
table=data.table(read.csv("tables/21_measurementErrors.csv",header = FALSE,check.names = FALSE))
...
$ROU$ is not included int his table because ROU is created in this step in a slighlty different way. 
...
table=data.table(read.csv("tables/22_sua_balancedW.csv",check.names = FALSE))
...
table=data.table(read.csv("tables/24_sua_balancedF.csv",check.names = FALSE))
...
table=data.table(read.csv("tables/25_FbsAggregationChina.csv"))
...
Nevertheless, this methodology has been used for producung FBS of year 2016. 
