...
t15_cap <- table_nums(name = "t15", 
                        caption = "Commodity Tree Wheat China Mainland, 2014")
...
t4_cap <- table_nums(name = "t4", 
                        caption = "Commodity Tree Wheat China Mainland, 2014 with weights")
...
t14_cap <- table_nums(name = "t14", 
                        caption = "Sua Balanced (Example China Mainland, Wheat 2014)")
...
The Aggregation of the SUAs into FBSs requires a structured and clear set of relationships between commodity and this representation is given by the *Commodity tree*.
The majority of the commodities are produced from one (or more) *parent* commodity (/ies) and/or are themselfes parent of one (or more) *child* (*children*) commodity (/ies). These relationships creates an intense and articulated network of relationships at different levels: The primary commodity, like crops, are *zero-level* commodities from which *level1* commodities are produced, which are in turn used to poduce other commodities of a gradually lower level. These network is known as *Commodity tree*. With the same name also the single *"trees"*, one for each linked processed product starting from the same primary commodity, are named, therefore there are as many commodity trees as many process chain in country. For a more detailed description of commodity trees, please see the specific documentation^[The reference document, at the moment is the *tecnical conversion factor* document available in the documentation folder on [*GitHub*](https://github.com/SWS-Methodology/faoswsStandardization/tree/master/documentation)]. Here is important to remember some fundamental charachteristics of commodity trees: 

 1. Each commodity tree is represented as a flowchart of the kind presentef in Figure \ref{fig:f2} where:
...
> where $availability_{p(c)}$ is the availability of each parent $p$ of child $c$ expressed in terms of $c$ (as say in *child equivalent*). The availability $availability_{p(c)}$ can be defined in different ways during different steps of the overall process. In general terms, it represents a difference between supply and utilizations, but utilizations may be or not be all included in the calculation. 
...
knitr::kable(table,caption = "Commodity Tree Wheat China Mainland, 2014",
...
knitr::kable(table,format="latex",caption = "Sua table with Production filled/incremented (Example China Mainland, Wheat 2014)",
             align = c("r",rep("l",length(colnames(table))-1))) %>% 
  kable_styling(font_size = 18,latex_options=c("scale_down", full_width = T))%>%
  row_spec(0, bold=TRUE, align = c("r",rep('c',times=length(colnames(table))-1))) %>%
 ...
 * $s_{cp}$ is the *share* of child $c$ from parent $p$. Shares here are defined as: 
...
The following Table reports the commodity tree of the wheat/China example with the extracolumn reporting the *weight* of each child-commodity.

```{r t4, fig.cap=t4_cap}
table=data.table(read.csv("tables/04_foodproc_calc.csv"))
knitr::kable(table,caption = "Commodity Tree Wheat China Mainland, 2014 with weights",
             align = c("r",rep("l",length(colnames(table))-1)))
...
`r table_nums("t5",display = "cite")` shows the SUA table after the two steps just described. Notice that, in the specific example, there was no need for the creation/incrementation of Production on any derived commodity because $Imb1$ is positive for all commodities. 
...
> Besides the excluded elements, there are 2 utilizations empty: *food* and *Industrial Use*. The Values of these two elements is calculated as folows and the result shown in `r table_nums("t13",display = "cite")`: 
...
The table resulting after the Sua Filling is called *Sua Balanced*. For the Example of China Mainland, wheat 2014 the *Sua Balanced* is reported in `r table_nums("t14",display = "cite")`

```{r t14, fig.cap=t14_cap}
table=data.table(read.csv("tables/14_sua_Balanced.csv"))
landscape(knitr::kable(table,format="latex",caption = "Sua Balanced (Example China Mainland, Wheat 2014)",
             align = c("r",rep("l",length(colnames(table))-1)))) %>% 
  kable_styling(font_size = 18,latex_options=c("scale_down", full_width = T))%>%
  row_spec(0, bold=TRUE, align = c("r",rep('c',times=length(colnames(table))-1))) %>%
