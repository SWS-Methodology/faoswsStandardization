...
figures inside a so-called \emph{Sua Unbalanced}. In this initial
account, food processing and ROU figures are not available (because
they, by default, will be measured during the process), whereas the
figures for all other variables have been already collected, imputed and
estimated through a specific \emph{module} (Figure \ref{fig:f1}).
...
\tightlist
\item
  expressing all elements \(El_{k,c}\) of the children of a parent
...
definition of the percentages of \emph{Measurement Errors (\(p_{ME}\))}
for each element as reported in the following Table:
