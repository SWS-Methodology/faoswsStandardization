...
These pieces of information have to be given externally, as the process is automated and there is non-human knowledge intervening in training the machine that is performing the calculations. The previous methodology used for compiling FBS made massive use of manual interventions while the new methodology tries to avoid it, but taking advantage of the work and effort of the country experts that compiled the FBSs in the past. Past information is embedded in the new methodology through the use of the *Utilization Table*. These tables  help identifying which variables have to be filled, commodity by commodity. Moreover, the amount of them to be assigned to figures is identified through a set of rules. This procedure makes use of the imbalance between supply and utilization values for each commodity and fills the figures that have been recognized as missing from the ***Utilization Table***. The filling procedure assigns figures using an ad-hoc algorithm designed for dimensioning the figures in accordance with the importance they had in the past FBSs. This *importance* is suggested by a weight given as a percentage based on the median value that each utilization had in the time range 2000 - 2013. All the commodities entering the *Sua Filling* procedure are eventually balanced.  

Not all the commodities enter the process the same way. In particular, while derived commodities always enter the filling procedure, primary commodities are not changed when the change might affect Production and trade figures. Indeed, Trade and production figures are normally published before the FBSs are calculated, therefore theis values cannot be modifyed. 

Figure \ref{fig:f4} shows the general Workflow of the *Sua Filling* highlighting the distinction between primary and derived commodities.
...
If this happens, it means that there is not enough supply for the commodity to be used in the way the SUA figures suggest. In this case, as shown in Figure \ref{fig:f6} the algorithm tries to solve the excess of utilization by reducing all the existing Utilizations by 30%, except Exports and Food Processing. The reason for exluding Food PRocessing is that this variable is linked with production of the following levels and to modify it would breack thatlink.Stock is changed in this case, because this is just a proportional reduction of values and does not need a time series analysis.

This steps affects any commodity with negative imbalance (either Primary or derived). If this is enough to cover the Utilization, the balancing line for that commodity is balanced and the process can go to the next step. Instead, after this reduction, there might be 2 different scenarios: 

 1. If this imbalance is still negative and the commodity is a primary commodity, the SUA line remains unbalanced. 
 
 2. If the imbalance is still negative but the commodity is a derived commodity, it is checked if the production figure is "*protected*" or not: 
...
> -- If Production figure is protected, a warning is given that tells that there is an unsolvable problem and that all figures have to be checked because either the production protected figure is wrong, or the Trade or some utilization have to be changed. 
 ...
 3. If the imbalance becomes positive, the line falls into the Scenario C that will be explained shortly.
